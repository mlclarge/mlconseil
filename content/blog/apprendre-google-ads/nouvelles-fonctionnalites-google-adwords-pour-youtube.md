---
title: "Campagnes vidéos publicitaires Youtube et Adwords"
date: 2013-02-06
author: "admin"
categories: ["Apprendre Google Ads"]
tags: []
slug: "nouvelles-fonctionnalites-google-adwords-pour-youtube"
---

La Saint Valentin approche *(a inspiré l'illustration de ce post :) )*, et l'union entre Youtube et Adwords perdure ! J'ai déjà abordé le sujet de l'optimisation vidéo pour les moteurs de recherche. Aujourd'hui, place à la **promotion sur Youtube et le réseau display**. Alors que Google lance officiellement , un [adwords universel](http://www.google.fr/adwords/enhancedcampaigns/features/), qui permet au sein d'une même campagne de maitriser , appareils, position géographique et timing de la journée ,  la firme Californienne met également le paquet pour booster sa **plateforme sociale de vidéos**  *(panneaux 4x3 rue, métro parisien lors du lancement d'une chaine de sport)* . Sur ce même sujet d'ailleurs,  les fournisseurs d’accès et l’Etat parlent actuellement à l’unisson pour faire comprendre à Google qu’il doit mettre la main à la poche s’il veut continuer à se servir des tuyaux Frenchis et de leurs contenus ,générés,  entre autre, par les internautes  et la presse Française ! Bref, sans faire de politique, ce n’est pas le sujet, parlons des nouvelles fonctionnalités disponibles depuis quelques jours sur l’interface Adwords associées à la chaine vidéo YT.

## Association Chaine Youtube et Adwords

Depuis l’année dernière, Google a intégré entièrement la gestion publicitaire Youtube à Adwords, pour plus de transparence et de simplicité pour les annonceurs. Donc,  pour annoncer sur la plateforme Youtube mais également sur le réseau de contenu « display » de Google, il faut passer par l’interface Adwords. Il faut opérer une** association de comptes**, très intuitive et faite en 3 clics de souris (voir association ci-dessous).

[![association-310x183.gif](/images/blog/association-310x183.gif) Association Adwords et Youtube
## Deux Nouveaux indicateurs de suivi de sa campagne vidéo

**L’audience** tout d’abord, indique la quantité d’internautes qui ont été exposés à l’annonce vidéo. Cet indicateur est donné par tranche de fois multiplié  par le nombre de personnes. Cela veut être 20 personnes soumises 3 fois à la même annonce.

**La fréquence**, elle est un ratio qui indique l’intérêt d’un internaute sur le message publicitaire. Si un taux de 1.5 apparait, cela veut dire que le message intéresse son destinataire, 1 indique plutôt un standard, moyen. En effet, c’est plutôt rare qu’un internaute souhaite revoir une même vidéo de publicité plus d’une fois.

[![audience-et-frequence-adwords-310x163.gif](/images/blog/audience-et-frequence-adwords-310x163.gif) Audience et fréquence  devisionnages
## Accès spécifiques dédiés aux objectifs marketing 

Quatre types de raccourcis voient le jour et qui permettent un accès en 1 clic, selon les axes de sa stratégie pubs vidéo, aux curseurs suivants :

	- **Visionnage **: vues et interactions sur vos vidéos (vues, cv, spectateurs uniques)

	- **Audience** : évolution de l’audience sur YTube (vues et abonnements consécutifs)

	- **Branding** : notoriété de la marque (impressions, fréquences de vues et d’impressions)

	- **Trafic et conversions** : clics et conversions sur le site web (taux de conversion, couts par conversions selon le nombre de visionnages)

[![indicateurs-adwords--310x144.gif](/images/blog/indicateurs-adwords--310x144.gif) indicateurs stratégiques en 1 clic

Reste à concevoir un c**ontenu vidéo de qualité**, d’ailleurs à ce propos *(annonce locale Bordelaise !)*, je signale une conférence à Bordeaux le 12 février,  par l’**agence de communication  Inaativ**,  animée par Vincent Pereira, sur le thème : [comment utiliser les vidéos pour augmenter sa visibilité sur internet](http://www.formateurconsultant.com/article-conference-gratuite-a-bordeaux-comment-utiliser-les-videos-pour-augmenter-sa-visibilite-sur-intern-114780102.html)
Du contenu à sa diffusion, il ne reste plus qu’à passer à l’action. Silence ! Ça tourne !